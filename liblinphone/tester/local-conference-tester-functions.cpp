 ...
 *		1) Main Stream:
 *			- Not Listener while requesting screen sharing (refused)
 *				-OR- Remote params of current Focus doesn't share screen
 ...
 *			- Is screen sharing OR camera is activated
 *			- VideoDirection has Send flag
 *
...
size_t compute_no_video_streams_2(const std::list<std::shared_ptr<Call>> &calls,
                                  const std::list<std::shared_ptr<ParticipantInfo>> &participants,
                                  const std::shared_ptr<Call> &currentCall,
                                  const std::shared_ptr<Core> &focus) {
	...
	if (currentCallParams->videoEnabled()) {
		...
		if (focus) {
			...
			if (currentFocusCall)
				remoteFocusScreenSharing = currentFocusCall->getRemoteParams()->screenSharingEnabled();
		}

		// 1) Main Stream
		if (localVideoLayout == ConferenceLayout::ActiveSpeaker) {
			bool isLocalScreenSharing = localParams->screenSharingEnabled();
			if (currentVideoDirection != LinphoneMediaDirectionInactive &&
			    (!(isLocalScreenSharing && currentRole == Participant::Role::Listener) || !remoteFocusScreenSharing))
				++nbVideoStreams; // Main
			if (currentRole == Participant::Role::Speaker && (currentVideoDirection & LinphoneMediaDirectionSendOnly) &&
			    ...
			    (!isScreenSharing || currentCameraEnabled))
				++nbVideoStreams; // Thumbnail
		} else {                  // Grid
			if (currentRole != Participant::Role::Listener && (isScreenSharing || currentCameraEnabled) &&
		...
		for (const auto &call : calls) {
			...
			auto address = call->getCore()->getIdentityAddress();
			...
			const auto localCallParams = call->getParams();
			const auto participantRole =
			    (itParticipant == participants.end() ? Participant::Role::Unknown : (*itParticipant)->getRole());
			const auto participantCurrentParams = call->getCurrentParams();
			if (((participantRole != Participant::Role::Listener) &&
			     (localCallParams->getVideoDirection() & LinphoneMediaDirectionSendOnly) &&
...
size_t compute_no_video_streams(const std::list<std::shared_ptr<Call>> &calls,
                                const std::list<std::shared_ptr<ParticipantInfo>> &participants,
                                const std::shared_ptr<Call> &currentCall) {
	return compute_no_video_streams_2(calls, participants, currentCall, nullptr);
}
...
void wait_for_conference_streams(std::initializer_list<std::reference_wrapper<CoreManager>> coreMgrs,
	...
	for (auto mgr : conferenceMgrs) {
		...
		CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(30), [mgr, &focus, &members, confAddr, enable_video,
				...
				for (auto call : calls) {
					if (call) {
						...
						if (!!linphone_config_get_int(linphone_core_get_config(mgr->lc), "misc",
							...
							nb_video_streams = compute_no_video_streams_2(
							    participantCalls, participantInfos, Call::getSharedFromThis(call), focus->lc->cppPtr);
						} else {
			...
			if (pconference) {
				...
				if (mgr == focus.getCMgr()) {
					...
					if (participant_call) {
						...
						BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(
						    participant_call, no_streams_audio, no_active_streams_video, no_streams_text));
						const LinphoneCallParams *call_lparams = linphone_call_get_params(participant_call);
					...
					if (focus_call) {
						BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(
						    focus_call, no_streams_audio, no_active_streams_video, no_streams_text));
						if (!enable_ice) {
				...
				if (mgr == focus.getCMgr()) {
					...
					if (focus_call) {
						_linphone_call_check_max_nb_streams(focus_call, no_streams_audio, no_max_streams_video,
						                                    no_streams_text);
						BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(focus_call, no_streams_audio,
						                                                      no_streams_video, no_streams_text));
						const LinphoneCallParams *call_lparams = linphone_call_get_params(focus_call);
						BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_lparams), enabled, int, "%0d");
						const LinphoneCallParams *call_rparams = linphone_call_get_remote_params(focus_call);
...
void create_one_participant_conference_toggle_video_base(LinphoneConferenceLayout layout,
	...
	{ // to make sure focus is destroyed after clients.
		...
		if (marie_calls_focus) {
			...
			BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(marie_calls_focus, no_streams_audio,
		...
		if (focus_called_by_marie) {
			_linphone_call_check_max_nb_streams(focus_called_by_marie, no_streams_audio, no_streams_video,
			                                    no_streams_text);
			BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(focus_called_by_marie, no_streams_audio,
			                                                      no_active_streams_video, no_streams_text));
		...
		if (marie_calls_focus) {
			_linphone_call_check_max_nb_streams(marie_calls_focus, no_streams_audio, no_streams_video, no_streams_text);
			BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(marie_calls_focus, no_streams_audio, no_streams_video,
			                                                      no_streams_text));
			const LinphoneCallParams *call_lparams = linphone_call_get_params(marie_calls_focus);
			BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_lparams));
			const LinphoneCallParams *call_rparams = linphone_call_get_remote_params(marie_calls_focus);
		...
		for (auto mgr : conferenceMgrs) {
			...
			if (mgr != focus.getCMgr()) {
				...
				if (focus_call) {
					...
					BC_ASSERT_TRUE(_linphone_call_check_nb_active_streams(focus_call, 1, (toggle_video) ? 4 : 0, 0));
				}
