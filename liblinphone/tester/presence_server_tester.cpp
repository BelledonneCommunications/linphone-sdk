...
static void publish_with_expire_timestamp_refresh_base(bool_t refresh_timestamps,
	...
	BC_ASSERT_TRUE(wait_for(marie->lc, pauline->lc, &marie->stat.number_of_LinphonePublishOk,
	                        marie_stat.number_of_LinphonePublishOk + 1));
	BC_ASSERT_TRUE(wait_for(marie->lc, pauline->lc, &pauline->stat.number_of_LinphoneConsolidatedPresenceOnline,
	                        pauline_stat.number_of_LinphoneConsolidatedPresenceOnline + 1));
...
static void notify_friend_capabilities_after_publish(void) {
	...
	BC_ASSERT_TRUE(
	    wait_for_list(lcs, &marie->stat.number_of_NotifyPresenceReceived, 3, liblinphone_tester_sip_timeout));
