...
std::optional<TlsCertificate> Core::findTlsCertInIndexedAuthInfosWithSubject(const std::string &username,
	...
	for (auto &authInfo : authInfos) {
		if (authInfo->isSuitableForChallenge(LinphoneAuthTls, username, "", domain, "") > 0) {
			...
			bctbx_x509_certificate_t *local_cert = nullptr;
			bctbx_signing_key_t *local_key = nullptr;
			bctbx_ext_signing_key_ref_t *local_key_ref = nullptr;

			if (!tlsCert.empty()) {
				if (!tlsKey.empty()) {
					local_cert = bctbx_x509_certificate_new();
					if (bctbx_x509_certificate_parse(local_cert, L_STRING_TO_C(tlsCert), tlsCert.size() + 1) != 0) {
						bctbx_x509_certificate_free(local_cert);
						local_cert = nullptr;
					}
					local_key = bctbx_signing_key_new();
					if (bctbx_signing_key_parse(local_key, L_STRING_TO_C(tlsKey), tlsKey.size() + 1, nullptr, 0) != 0) {
						bctbx_signing_key_free(local_key);
						local_key = nullptr;
					}
				} else if (!bctbx_ext_signing_key_ref_empty(&authInfo->getExtTlsKeyRef())) {
					local_cert = bctbx_x509_certificate_new();
					if (bctbx_x509_certificate_parse(local_cert, L_STRING_TO_C(tlsCert), tlsCert.size() + 1) != 0) {
						bctbx_x509_certificate_free(local_cert);
						local_cert = nullptr;
					}
					local_key_ref = bctbx_ext_signing_key_ref_clone(&authInfo->getExtTlsKeyRef());
				}
			} else if (!tlsCertPath.empty() && !tlsKeyPath.empty()) {
				// if auth info holds a tls_cert_path and key_path, it is assumed they are files
				local_cert = bctbx_x509_certificate_new();
				if (bctbx_x509_certificate_parse_file(local_cert, L_STRING_TO_C(tlsCertPath)) != 0) {
					bctbx_x509_certificate_free(local_cert);
					local_cert = nullptr;
				}
				local_key = bctbx_signing_key_new();
				if (bctbx_signing_key_parse_file(local_key, L_STRING_TO_C(tlsKeyPath), nullptr) != 0) {
					bctbx_signing_key_free(local_key);
					local_key = nullptr;
				}
			}
			if (local_cert != nullptr && (local_key != nullptr || local_key_ref != nullptr)) {
				if (bctbx_x509_certificate_subject_match(local_cert, L_STRING_TO_C(subject)) == TRUE) {
					tlsCertificate.certificatePem = local_cert;
					tlsCertificate.keyPem = local_key;
					tlsCertificate.keyRef = local_key_ref;
					auto *fingerprint = static_cast<char *>(bctbx_malloc0(200));
					if (bctbx_x509_certificate_get_fingerprint(local_cert, fingerprint, 200, BCTBX_MD_UNDEFINED) <= 0) {
						bctbx_free(fingerprint);
						fingerprint = nullptr;
					} else {
						tlsCertificate.fingerprint = L_C_TO_STRING(fingerprint);
						bctbx_free(fingerprint);
			...
			bctbx_x509_certificate_free(local_cert);
			bctbx_signing_key_free(local_key);
			bctbx_ext_signing_key_ref_free(local_key_ref);
		}
	}
	return nullopt;
}
...
Core::fillBelleSipAuthEvent(belle_sip_auth_event *event, const std::string &username, const std::string &domain) {
	...
	switch (belle_sip_auth_event_get_mode(event)) {
		...
		case BELLE_SIP_AUTH_MODE_TLS: {
			...
			string tlsCertPath;
			string tlsKeyPath;
			bctbx_ext_signing_key_ref_t tlsKeyRef;
			...
			if (authInfo) { /* tls_auth_info found something */
				...
				if (!tlsCert.empty() && tlsKey.empty()) {
					tlsKeyRef = authInfo->getExtTlsKeyRef();
				}
				if (tlsCert.empty() || (tlsKey.empty() && bctbx_ext_signing_key_ref_empty(&tlsKeyRef))) {
			...
			} else if (!tlsCert.empty() && !bctbx_ext_signing_key_ref_empty(&tlsKeyRef)) {
				belle_sip_certificates_chain_t *bs_tlsCert = belle_sip_certificates_chain_parse(
				    tlsCert.c_str(), tlsCert.size(), BELLE_SIP_CERTIFICATE_RAW_FORMAT_PEM);
				if (bs_tlsCert) {
					belle_sip_auth_event_set_signing_key(event, belle_sip_signing_new_key_ref(&tlsKeyRef));
					belle_sip_auth_event_set_client_certificates_chain(event, bs_tlsCert);
				}
			} else if (!tlsCertPath.empty() && !tlsKeyPath.empty()) {
