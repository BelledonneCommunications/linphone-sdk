...
public:
	...
	friend void ::linphone_core_add_friend_list(LinphoneCore *core, LinphoneFriendList *list);
	...
	friend LinphoneFriend * ::linphone_core_find_friend_by_out_subscribe(const LinphoneCore *lc,
	                                                                     LinphonePrivate::SalOp *op);
	friend LinphoneFriend * ::linphone_core_find_friend_by_phone_number(const LinphoneCore *core,
	                                                                    const char *phone_number);
	friend void ::linphone_core_invalidate_friend_subscriptions(LinphoneCore *lc);
	friend void ::linphone_core_remove_friend_list(LinphoneCore *core, LinphoneFriendList *list);
	...
	getVcardListFromBuffer(const std::string &vcardBuffer) const;
	std::pair<LinphoneStatus, std::vector<std::shared_ptr<Vcard>>>
	getVcardListFromVcard4File(const std::string &vcardFile) const;
...
private:
	...
	LinphoneStatus importFriendsFromVcard4(const std::vector<std::shared_ptr<Vcard>> &vcards);
	...
	void removeFromDb();
	void saveInDb(bool saveFriends = false);
	...
	void synchronizeFriendsFromServerVcard4();
	void synchronizeFriendsFromServerCardDav();

	static void
	subscriptionStateChanged(LinphoneCore *lc, std::shared_ptr<Event> event, LinphoneSubscriptionState state);
#ifdef VCARD_ENABLED
...
class FriendListCbs : public bellesip::HybridObject<LinphoneFriendListCbs, FriendListCbs>,
                      public ListenerBase,
                      public UserDataAccessor {
public:
	// Getters
