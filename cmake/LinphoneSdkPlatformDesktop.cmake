...
#remove it when mswinrtvid is no more needed
	if(ENABLE_MSWINRTVIDEO)
		...
		ExternalProject_Add(sdk-mswinrtvideo
			...
			CMAKE_GENERATOR "${SYSTEM_GENERATOR}"
			CMAKE_ARGS "-DLINPHONESDK_DIR=${LINPHONESDK_DIR}" "-DLINPHONESDK_BUILD_DIR=${CMAKE_BINARY_DIR}/desktop" "-DLINPHONESDK_VERSION=${LINPHONESDK_VERSION}" "-DLINPHONESDK_STATE=${LINPHONESDK_STATE}" "-DLINPHONESDK_WINDOWS_BASE_URL=${LINPHONESDK_WINDOWS_BASE_URL}" "-DLINPHONE_PLATFORM=x86" ${_cmake_args}
			#CMAKE_CACHE_ARGS ${_cmake_cache_args}
			BUILD_COMMAND ${CMAKE_COMMAND} -E echo ""
			INSTALL_COMMAND ${CMAKE_COMMAND} -E echo ""
		)
	...
	elseif(WIN32)
		...
		add_custom_command(TARGET sdk
			COMMENT "Generating the SDK (zip file)"
			COMMAND "${CMAKE_COMMAND}" "-DLINPHONESDK_PLATEFORM=${CMAKE_GENERATOR_PLATFORM}" "-DLINPHONESDK_DIR=${LINPHONESDK_DIR}" "-DLINPHONESDK_BUILD_DIR=${CMAKE_BINARY_DIR}" "-DLINPHONESDK_VERSION=${LINPHONESDK_VERSION}" "-DLINPHONESDK_STATE=${LINPHONESDK_STATE}" "-DLINPHONESDK_WINDOWS_BASE_URL=${LINPHONESDK_WINDOWS_BASE_URL}" "-DLINPHONESDK_ENABLED_FEATURES_FILENAME=${CMAKE_BINARY_DIR}/enabled_features.txt" "-DCMAKE_INSTALL_PREFIX=${_desktop_install_prefix}"
			"-P" "${LINPHONESDK_DIR}/cmake/Windows/GenerateSDK.cmake"
		)
	endif()
endif()	
