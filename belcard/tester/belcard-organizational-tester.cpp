...
static void logo_property(void) {
	...
	test_property<BelCardLogo>("LOGO:data:image/"
	                           "jpeg;base64,"
	                           "MIICajCCAdOgAwIBAgICBEUwDQYJKoZIhvcAQEEBQAwdzELMAkGA1UEBhMCVVMxLDAqBgNVBAoTI05ldHNjYXBl"
	                           "IENvbW11bmljYXRpb25zIENvcnBvcmF0aW9uMRwwGgYDVQQLExNJbmZvcm1hdGlvbiBTeXN0\r\n");
}
...
static void related_property(void) {
	...
	test_property<BelCardRelated>(
	    "RELATED;TYPE=co-worker;VALUE=text:Please contact my assistant Jane Doe for any inquiries.\r\n");
}

static test_t tests[] = {
    TEST_NO_TAG("TITLE", title_property),   TEST_NO_TAG("ROLE", role_property),
    TEST_NO_TAG("LOGO", logo_property),     TEST_NO_TAG("ORG", org_property),
    TEST_NO_TAG("MEMBER", member_property), TEST_NO_TAG("RELATED", related_property),
};

test_suite_t vcard_organizational_properties_test_suite = {
    "Organizational", NULL, NULL, NULL, NULL, sizeof(tests) / sizeof(tests[0]), tests};
