...
// Static old handlers : Use static and not capture in lambda because if a capture is present, the lambda cannot be converted to a function pointer and then, it is unusable by ahndlers 
...
LONG WINAPI _unhandled_exceptions_hook(_EXCEPTION_POINTERS* pExp) { _signal_hook(SIGABRT); if(gOldUnhandledException) return gOldUnhandledException(pExp); else return EXCEPTION_EXECUTE_HANDLER; }
void _abrt_signal_hook( int u){ _signal_hook(u); if(gOldABRT ) gOldABRT(u); }
void _fpe_signal_hook( int u){ _signal_hook(u); if(gOldFPE ) gOldFPE(u); }
void _ill_signal_hook( int u){ _signal_hook(u); if(gOldILL ) gOldILL(u); }
void _egv_signal_hook( int u){ _signal_hook(u); if(gOldEGV) gOldEGV(u); }
void _term_signal_hook( int u){ _signal_hook(u); if(gOldTERM ) gOldTERM(u); }

void _terminate_hook(){ _signal_hook(SIGTERM); if(gOldTerminateHandler) gOldTerminateHandler(); else std::abort(); }
...
// Try to take account of all handlers
	...
	// Top level exception
		...
		if(gOldABRT)
			setSignalHook(SIGABRT, gOldABRT, gOldABRT); // Abnormal termination
		gOldABRT = NULL;
		if(gOldFPE)
			setSignalHook(SIGFPE, gOldFPE, gOldFPE); // Floating-point error
		gOldFPE = NULL;
		if(gOldILL)
			setSignalHook(SIGILL, gOldILL, gOldILL); // Illegal instruction
		gOldILL = NULL;
		if(gOldEGV)
			setSignalHook(SIGSEGV, gOldEGV, gOldEGV); // Illegal storage access
		gOldEGV = NULL;
		if(gOldTERM)
			setSignalHook(SIGTERM, gOldTERM, gOldTERM); // Terminate request
		gOldTERM = NULL;
	// Termination
		if(gOldTerminateHandler)
			setTerminateHook(gOldTerminateHandler,gOldTerminateHandler );
		gOldTerminateHandler = NULL;
	// Top level exception
		if(gOldUnhandledException)
			setUnhandledExceptionHook(gOldUnhandledException, gOldUnhandledException);
