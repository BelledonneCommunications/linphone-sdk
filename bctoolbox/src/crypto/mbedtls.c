	...
	while (ssl_ctx->ssl_ctx.state != MBEDTLS_SSL_HANDSHAKE_OVER) {
		...
		if (ssl_ctx->callback_cli_cert_function != NULL) { /* check we have a callback function */
			...
			if (ssl_ctx->ssl_ctx.state == MBEDTLS_SSL_CLIENT_CERTIFICATE && ssl_ctx->ssl_ctx.client_auth > 0) {
				...
				bctbx_list_t *names = bctbx_x509_certificate_get_subjects(
				    (const bctbx_x509_certificate_t *)ssl_ctx->ssl_ctx.session_negotiate->peer_cert);
...
bctbx_ssl_set_hs_own_cert(bctbx_ssl_context_t *ssl_ctx, bctbx_x509_certificate_t *cert, bctbx_signing_key_t *key) {
	/* WARNING Dirty-Dirty trick : in mbedtls ssl_set_hs_own_cert shall be caller on server side only for SNI, there is
	 * no real equivalent on client side yet */
	...
	} else { /* if we are client, call the conf function on the function in use inside the ssl_context, dirty but it
		        works for now */
		return mbedtls_ssl_conf_own_cert((mbedtls_ssl_config *)ssl_ctx->ssl_ctx.conf, (mbedtls_x509_crt *)cert,
