 ...
 * @param[in]	recursive	if false, the directory must be empty to be deleted otherwise recursively delete with all
 *content
 *
 * @return 0 on success
 **/
 ...
 *  notes:
 *     - M suffix(month) is consired a 30 days period without any consideration of the current date, Y is always 365
 * days.
...
BCTBX_PUBLIC ssize_t bctbx_recvfrom(
...
BCTBX_PUBLIC int
bctbx_getaddrinfo(const char *node, const char *service, const struct addrinfo *hints, struct addrinfo **res);
BCTBX_PUBLIC void bctbx_freeaddrinfo(struct addrinfo *res);
BCTBX_PUBLIC int bctbx_getnameinfo(const struct sockaddr *address,
...
BCTBX_PUBLIC int
bctbx_addrinfo_to_printable_ip_address(const struct addrinfo *ai, char *printable_ip, size_t printable_ip_size);
BCTBX_PUBLIC int
bctbx_sockaddr_to_ip_address(const struct sockaddr *sa, socklen_t salen, char *ip, size_t ip_size, int *port);
BCTBX_PUBLIC int bctbx_sockaddr_to_printable_ip_address(struct sockaddr *sa,
                                                        socklen_t salen,
                                                        char *printable_ip,
...
/**
 * This function will transform a V4 to V6 mapped address to a pure V4 and write it into result, or will just copy it
 ...
 **/
BCTBX_PUBLIC void
bctbx_sockaddr_remove_nat64_mapping(const struct sockaddr *v6, struct sockaddr *result, socklen_t *result_len);
 ...
 **/
BCTBX_PUBLIC void
bctbx_sockaddr_ipv6_to_ipv4(const struct sockaddr *v6, struct sockaddr *result, socklen_t *result_len);
 ...
 **/
BCTBX_PUBLIC void
bctbx_sockaddr_ipv4_to_ipv6(const struct sockaddr *v4, struct sockaddr *result, socklen_t *result_len);
 ...
 * @param[in]	input_string			The input string buffer, must be hexadecimal(it is not checked by function, any
 * non hexa char is converted to 0)
...
/**
 * @brief Convert an unsigned 32 bits integer into the corresponding hexadecimal string(including null termination
 * character)
 *
 * @param[out]	output_string		The output string buffer, must have a length of at least 9 bytes(8 nibbles and the
 * '\0')
 ...
 *
 * Note : there is no check on the length or validity as an hexa string on the input, incorrect byte is silently mapped
 * to 0
 */
BCTBX_PUBLIC uint32_t bctbx_str_to_uint32(const uint8_t *input_string);

/**
 * @brief Convert an unsigned 64 bits integer into the corresponding hexadecimal string(including null termination
 * character)
 *
 * @param[out]	output_string		The output string buffer, must have a length of at least 17 bytes(16 nibbles and the
 * '\0')
 ...
 * @param[in] if TRUE, prepends handlers with bctoolbox hooks. If FALSE, remove them and replace by old hooks (stored
 * from the last call of bctbx_set_stack_trace_hooks() ). By default, it is FALSE.
 */
