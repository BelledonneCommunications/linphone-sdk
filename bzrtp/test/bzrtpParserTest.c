...
uint8_t maliciousHelloPacketZrtpHashFragment1[96] = {
0x11, 0x00, 0x00, 0x01,
0x5a, 0x52, 0x54, 0x50,
0x76, 0xdc, 0xa7, 0xbd,
0x00, 0x00, 0x00, 0x26, // message Id is 0. Check we do no try to write in a non allocated memory space
0x00, 0x00, 0x00, 0x12, // offset 0, fragment size 0x12 (18 words)
0x50, 0x5a, 0x00, 0x26,
0x48, 0x65, 0x6c, 0x6c,
0x6f, 0x20, 0x20, 0x20,
0x31, 0x2e, 0x31, 0x30,
...
0x20, 0x12, 0xa2, 0xf0,
0x6c, 0xfd, 0x0f, 0xfb,
0x53, 0x96, 0xaa, 0x51}; //CRC

uint8_t maliciousHelloPacketZrtpHashFragment2[96] = {
0x11, 0x00, 0x00, 0x01,
0x5a, 0x52, 0x54, 0x50,
0x76, 0xdc, 0xa7, 0xbd,
0x12, 0x34, 0x00, 0x26,  // message size is 0x26
0x00, 0x30, 0x00, 0x12, // offset 0x30: try to write after the message
0x50, 0x5a, 0x00, 0x26,
0x48, 0x65, 0x6c, 0x6c,
0x6f, 0x20, 0x20, 0x20,
0x31, 0x2e, 0x31, 0x30,
...
0x20, 0x12, 0xa2, 0xf0,
0x6c, 0xfd, 0x0f, 0xfb,
0xc1, 0xff, 0x9d, 0xeb}; //CRC

uint8_t HelloPacketZrtpHashFragment1[96] = {
0x11, 0x00, 0x00, 0x01,
0x5a, 0x52, 0x54, 0x50,
0x76, 0xdc, 0xa7, 0xbd,
0x12, 0x34, 0x00, 0x26, // message Id 0x1234 total len 0x26 (38 words)
0x00, 0x00, 0x00, 0x12, // offset 0, fragment size 0x12 (18 words)
0x50, 0x5a, 0x00, 0x26,
0x48, 0x65, 0x6c, 0x6c,
0x6f, 0x20, 0x20, 0x20,
...
0x20, 0x12, 0xa2, 0xf0,
0x6c, 0xfd, 0x0f, 0xfb,
0xf8, 0x99, 0x07, 0x69}; //CRC

uint8_t HelloPacketZrtpHashFragment2[104] = {
0x11, 0x00, 0x00, 0x01,
0x5a, 0x52, 0x54, 0x50,
0x76, 0xdc, 0xa7, 0xbd,
0x12, 0x34, 0x00, 0x26, // message Id 0x1234 total len 0x26 (38 words)
0x00, 0x12, 0x00, 0x14, // offset 0x12, fragment size 0x14 (20 words)
0x88, 0xd8, 0xe6, 0x52,
0x00, 0x02, 0x44, 0x51,
0x53, 0x33, 0x38, 0x34,
0x53, 0x32, 0x35, 0x36,
...
/* first parse a fragmented hello packet - just do the packet check as it is supposed to reassemble the packets
 * second try to parse a malicious fragmented packet - it shall fail */
static void test_fragmentedPacket(void) {
	bzrtpPacket_t *zrtpPacket;
	int retval;
	uint16_t inputLength;
	uint8_t *input;
	...
	/* try a malicious packet with message id 0 that would try to write in a non allocated space */
	inputLength = sizeof(maliciousHelloPacketZrtpHashFragment1);
	input = maliciousHelloPacketZrtpHashFragment1;
	zrtpPacket = bzrtp_packetCheck(&input, &inputLength, context12345678->channelContext[0], &retval);
	BC_ASSERT_PTR_NULL(zrtpPacket);
	BC_ASSERT_EQUAL(retval, BZRTP_PARSER_ERROR_INVALIDPACKET, int, "%d");
	/* try a malicious packet with message the offset past the packet size that would trigger a buffer overflow */
	inputLength = sizeof(maliciousHelloPacketZrtpHashFragment2);
	input = maliciousHelloPacketZrtpHashFragment2;
	zrtpPacket = bzrtp_packetCheck(&input, &inputLength, context12345678->channelContext[0], &retval);
	BC_ASSERT_PTR_NULL(zrtpPacket);
	BC_ASSERT_EQUAL(retval, BZRTP_PARSER_ERROR_INVALIDPACKET, int, "%d");

	/* try a valid 2 fragments hello message */
	inputLength = sizeof(HelloPacketZrtpHashFragment1);
	input = HelloPacketZrtpHashFragment1;
	zrtpPacket = bzrtp_packetCheck(&input, &inputLength, context12345678->channelContext[0], &retval);
	BC_ASSERT_PTR_NULL(zrtpPacket);
	BC_ASSERT_EQUAL(retval, BZRTP_PARSER_INFO_PACKETFRAGMENT, int, "%d");
	inputLength = sizeof(HelloPacketZrtpHashFragment2);
	input = HelloPacketZrtpHashFragment2;
	zrtpPacket = bzrtp_packetCheck(&input, &inputLength, context12345678->channelContext[0], &retval);
	BC_ASSERT_PTR_NOT_NULL(zrtpPacket);
	BC_ASSERT_EQUAL(retval, 0, int, "%d");
	/* parse the hello packet */
	retval = bzrtp_packetParser(context12345678, context12345678->channelContext[0], HelloPacketZrtpHash, sizeof(HelloPacketZrtpHash), zrtpPacket);
	BC_ASSERT_EQUAL(retval, 0, int, "%d");
...
static test_t packet_parser_tests[] = {
	...
	TEST_NO_TAG("Fragmented ZRTP packet", test_fragmentedPacket)
};
