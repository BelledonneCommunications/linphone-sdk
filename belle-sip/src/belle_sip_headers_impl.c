...
void belle_sip_header_address_set_quoted_displayname(belle_sip_header_address_t *address, const char *value) {
	...
	if (strlen(value) > 2) address->displayname = _belle_sip_str_dup_and_unquote_string(value);
	else address->displayname = NULL;
}
...
BELLE_SIP_NEW_HEADER(header_to, header_address, BELLE_SIP_TO) BELLE_SIP_PARSE(header_to)
...
BELLE_SIP_NEW_HEADER(header_diversion, header_address, BELLE_SIP_DIVERSION) BELLE_SIP_PARSE(header_diversion)
...
BELLE_SIP_NEW_HEADER(header_route, header_address, BELLE_SIP_ROUTE) BELLE_SIP_PARSE(header_route)
...
belle_sip_error_code belle_sip_header_www_authenticate_marshal(belle_sip_header_www_authenticate_t *www_authenticate,
	...
	if (www_authenticate->stale >= 0) {
		error = belle_sip_snprintf(buff, buff_size, offset, "%sstale=%s", border,
...
belle_sip_error_code belle_sip_header_authentication_info_marshal(
	...
	if (authentication_info->next_nonce) {
		error =
		    belle_sip_snprintf(buff, buff_size, offset, "%snextnonce=\"%s\"", border, authentication_info->next_nonce);
	}
	return error;
}
