...
#define SIP_URI_GET_SET_STRING_NO_ENCLOSING_BRACKETS(attribute)                                                        \
	GET_SET_STRING_NO_ENCLOSING_BRACKETS(belle_sip_uri, attribute)
#define SIP_URI_SET_ESCAPED_STRING(attribute) SET_ESCAPED_STRING(belle_sip_uri, attribute)
...
belle_sip_error_code belle_sip_uri_marshal(const belle_sip_uri_t *uri, char *buff, size_t buff_size, size_t *offset) {
...

	{
		...
		belle_sip_list_for_each2(uri->params.param_list, (void (*)(void *, void *))encode_params,
		                         &encparams->param_list);
...

	{
		...
		for (list = encheaders; list != NULL; list = list->next) {
			...
			if (list == encheaders) {
				// first case
				error = belle_sip_snprintf(buff, buff_size, offset, "?%s=%s", container->name,
...
int belle_sip_uri_get_listening_port(const belle_sip_uri_t *uri) {
	...
	if (port == 0) port = belle_sip_listening_point_get_well_known_port(transport);
	return port;
}
...
void belle_sip_uri_set_scheme(belle_sip_uri_t *obj, const char *value) {
	if (belle_sip_strcasecmp(value, "sips:")) {
		belle_sip_uri_set_secure(obj, 1);
	}
}
...
SIP_URI_SET_ESCAPED_STRING(user)
SIP_URI_GET_SET_STRING(user_password)
SIP_URI_SET_ESCAPED_STRING(user_password)
SIP_URI_GET_SET_STRING_NO_ENCLOSING_BRACKETS(host)
...
void belle_sip_uri_add_escaped_header(belle_sip_uri_t *obj, const char *header) {
	char *tmp = belle_sip_strdup(header);
	char *current = tmp;
	char *equal = strchr(current, '=');
	if (equal) {
		*equal = '\0';
		current = belle_sip_to_unescaped_string(belle_sip_trim_whitespaces(current));
		equal = belle_sip_to_unescaped_string(belle_sip_trim_whitespaces(equal + 1));
		belle_sip_uri_set_header(obj, current, equal);
		...
		if (strlen(current) > 0) {
			...
			belle_sip_uri_set_header(obj, current, NULL);
			belle_sip_free(current);
		}
	}
	belle_sip_free(tmp);
...
*/
static int
belle_sip_uri_equals_with_omission(const belle_sip_uri_t *uri_a, const belle_sip_uri_t *uri_b, bool_t uri_omitting) {
	...
	*/
	...
	if (uri_omitting) {
		...
		if (port_a == 0) {
			const char *transport_a =
			    belle_sip_parameters_get_case_parameter((belle_sip_parameters_t *)uri_a, "transport");
		...
		if (port_b == 0) {
			const char *transport_b =
			    belle_sip_parameters_get_case_parameter((belle_sip_parameters_t *)uri_b, "transport");
	...
	*/
	...
	for (params = belle_sip_parameters_get_parameters((belle_sip_parameters_t *)uri_a); params != NULL;
	     params = params->next) {
		a_param = (belle_sip_param_pair_t *)(params->data);
		if ((b_param_value = belle_sip_parameters_get_parameter((belle_sip_parameters_t *)uri_b, a_param->name)) !=
		    NULL) {
...
static uri_components_t uri_component_use_for_contact_in_reg = {
    "Contact in REG", o, o, /*m*/ o, o, o, na, o, o, o, na, o, o};
static uri_components_t uri_component_use_for_dialog_ct_rr_ro = {
    "Dialog Contact/R-R/Route", o, o, /*m*/ o, o, o, na, o, na, o, o, o, na};
...
#define CHECK_URI_COMPONENT(uri_component, uri_component_name, component_use_rule, component_use_rule_name)            \
	if (!check_component(uri_component, component_use_rule)) {                                                         \
		belle_sip_error("Uri component [%s] does not follow reqs [%s] for context [%s]", uri_component_name,           \
		                mark_to_string(component_use_rule), component_use_rule_name);                                  \
		return FALSE;                                                                                                  \
	}
...
static int check_uri_components(const belle_sip_uri_t *uri, const uri_components_t *components_use) {
	...
	CHECK_URI_COMPONENT(belle_sip_parameters_has_parameter(&uri->params, "maddr"), "maddr-param",
	                    components_use->maddr_param, components_use->name)
	CHECK_URI_COMPONENT(belle_sip_parameters_has_parameter(&uri->params, "ttl"), "ttl-param", components_use->ttl_param,
	                    components_use->name)
	CHECK_URI_COMPONENT(belle_sip_parameters_has_parameter(&uri->params, "transport"), "transp.-param",
	                    components_use->transp_param, components_use->name)
	CHECK_URI_COMPONENT(belle_sip_parameters_has_parameter(&uri->params, "lr"), "lr-param", components_use->lr_param,
	                    components_use->name)
	/*..*/
	CHECK_URI_COMPONENT(belle_sip_list_size(belle_sip_parameters_get_parameters(uri->header_list)) > 0, "headers",
	                    components_use->headers, components_use->name)
	return TRUE;
}
...
int belle_sip_uri_check_components_from_context(const belle_sip_uri_t *uri,
                                                const char *method,
	...
	else if (strcasecmp(BELLE_SIP_CONTACT, header_name) == 0 || strcasecmp(BELLE_SIP_RECORD_ROUTE, header_name) == 0 ||
