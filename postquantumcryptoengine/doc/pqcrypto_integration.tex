...
\begin{versionhistory}
  ...
  \vhEntry{1.1}{\today}{JP}{Instant messaging intergration and algorithm support update}
  \vhEntry{1.0}{September 20, 2022}{JP}{Initial version}
\end{versionhistory}
...
\paragraph*{}As of April 2026, \textit{liboqs} provides the following Key Encapsulation Mechanism (KEM) algorithms:
\begin{itemize}
  \item BIKE
...
\paragraph{}Linphone currently supports ML-KEM, Kyber(deprecated) and HQC, specifically the variants ML-KEM512, ML-KEM768, MLKEM-1024, Kyber512 and Kyber1024, HQC128 and HQC256. Other suitable, see \ref{subsec:zrtpRequirements}, algorithms provided by \textit{liboqs} can be included without major changes to the architecture.
...
\begin{algorithmic}
  ...
  \end{algorithm}
  \FloatBarrier
\paragraph*{}This module allows to combine any number and any set of KEM algorithms. The ZRTP module produce the followind set:
\begin{itemize}
  \item two algorithms combined: X25519/ML-KEM512, X25519/Kyber512, X25519/HQC128, X448/ML-KEM1024, X448/Kyber1024, X448/HQC256
  \item three algorithms combined: X25519/Kyber512/HQC128, X448/Kyber1024/HQC256
\end{itemize}
\paragraph*{}ML-KEM/Kyber and HQC are based on different mathematical problems, hence the interest of combining them, with the classic ECDH exchange, should one of them be broken in the future.

\subsection{Enable PQC in linphone SDK}
\paragraph*{}\textit{liboqs} is linked to a module called \href{https://gitlab.linphone.org/BC/public/linphone-sdk/-/tree/master/postquantumcryptoengine}{PostQuantumCryptoEngine}. To build the requested part of \textit{liboqs} and this module, call the linphone-sdk build command with the option
...
\paragraph*{}Instant messaging is end2end encrypted using Lime\citep{lime}, a Signal protocol derivative. This protocol originally relied on Elliptic-Curve Diffie-Hellman and EdDSA properties.
\paragraph*{}The PQ crypto support was then added to Lime before the publication by Signal of the PQXDH specification but it ended being very similar. Same goes for the PQ ratchet added to the Lime protocol ending being very similar to \href{https://security.apple.com/blog/imessage-pq3/}{PQ3} Apple protocol.
\paragraph*{}The Lime protocol does not combine PQKEM and ECDHKEM as described in this document. See its documentation for implementation and protocol details.
\newpage
