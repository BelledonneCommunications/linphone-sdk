...
shared_ptr<Recognizer> ABNFElement::buildRecognizer(const shared_ptr<Grammar> &grammar) {
	if (mElement) return mElement->buildRecognizer(grammar);
	if (!mRulename.empty()) return grammar->getRule(mRulename);
	if (!mCharVal.empty()) {
		if (mCharVal.size() == 1) return Foundation::charRecognizer(mCharVal[0], false);
		else return Utils::literal(mCharVal);
	}
...
shared_ptr<Recognizer> ABNFRuleList::buildRecognizer(const shared_ptr<Grammar> &grammar) {
	for (auto it = mRules.begin(); it != mRules.end(); ++it) {
		...
		if (rule->isExtension()) {
			 ...
			 * we should not add it a second time, otherwise the name of the recognizer and the name in the grammar
			 * entry will be different. To solve this problem, we use an intermediary AliasRecognizer*/
...
ABNFGrammarBuilder::ABNFGrammarBuilder() : mParser(make_shared<ABNFGrammar>()) {
...
shared_ptr<Grammar> ABNFGrammarBuilder::createFromAbnf(const string &abnf, const shared_ptr<Grammar> &gram) {
	...
	if (parsed < (size_t)abnf.size()) {
		bctbx_error("Only %llu bytes parsed over a total of %llu.", (unsigned long long)parsed,
		            (unsigned long long)abnf.size());
		return nullptr;
	}
