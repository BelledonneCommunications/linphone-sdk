...
typedef enum _WebRTCAECType { WebRTCAECTypeNormal, WebRTCAECTypeMobile } WebRTCAECType;
...
static void webrtc_aec_preprocess(MSFilter *f) {
	...
	if (s->aec_type == WebRTCAECTypeNormal) {
		...
		if (s->enable_delay_logging) aec_config.delay_logging = kAecTrue;
		else aec_config.delay_logging = kAecFalse;
...
static void webrtc_aec_process(MSFilter *f) {
	...
	while (ms_bufferizer_read(&s->echo, (uint8_t *)echo, (size_t)nbytes) >= (size_t)nbytes) {
		...
		if (s->aec_type == WebRTCAECTypeNormal) {
			...
			if (WebRtcAec_BufferFarend(s->aecInst, mswebrtc_aec_splitting_filter_get_ref(s->splitting_filter),
			                           (size_t)mswebrtc_aec_splitting_filter_get_bandsize(s->splitting_filter)) != 0)
				ms_error("WebRtcAec_BufferFarend() failed.");
			if (WebRtcAec_Process(s->aecInst, mswebrtc_aec_splitting_filter_get_echo_bands(s->splitting_filter),
		...
		// FIXME FHA
		if (s->enable_delay_logging) {
			int median = 0;
			int std = 0;
			float fraction_poor_delays = 0.;
			WebRtcAec_GetDelayMetrics(s->aecInst, &median, &std, &fraction_poor_delays);
			s->delay_median_ms = median;
			// // test: set estimated delay as current shift to apply to samples
			// if (median > 0 && s->echostarted) {
			// 	s->nominal_ref_samples = median * s->samplerate / 1000;
			// 	// 	ms_message("current delay metrics are %d, %d, %f, update samples ref with %d", median, std,
			// 	// 	           fraction_poor_delays, s->nominal_ref_samples);
			// 	// } else {
			// 	// 	ms_message("current delay metrics are %d, %d, %f", median, std, fraction_poor_delays);
			// }
		}
	}
}
...
static int webrtc_aec_set_sr(MSFilter *f, void *arg) {
	...
	if (sr != requested_sr)
		ms_message("Webrtc %s does not support sampling rate %i, using %i instead",
		           ((s->aec_type == WebRTCAECTypeNormal) ? "aec" : "aecm"), requested_sr, sr);
...
static MSFilterMethod webrtc_aec_methods[] = {{MS_FILTER_SET_SAMPLE_RATE, webrtc_aec_set_sr},
...
MSFilterDesc ms_webrtc_aec_desc = {MS_FILTER_PLUGIN_ID,    MS_WEBRTC_AEC_NAME,     MS_WEBRTC_AEC_DESCRIPTION,
...
MSFilterDesc ms_webrtc_aecm_desc = {MS_FILTER_PLUGIN_ID,     MS_WEBRTC_AECM_NAME,    MS_WEBRTC_AECM_DESCRIPTION,
