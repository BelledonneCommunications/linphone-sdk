...
double CU_get_elapsed_time(void)
{
  if (CU_TRUE == f_bTestIsRunning) {
	return ((double)time(0) - (double)f_start_time);
  }
  else {
...
CU_ErrorCode CU_run_all_tests(void)
{
  ...
  if (NULL == pRegistry) {
	...
	f_run_summary.ElapsedTime = ((double)time(0) - (double)f_start_time);
  ...
  if (NULL == pSuite) {
	...
	if (CU_FALSE != f_failure_on_inactive) {
	  add_failure(&f_failure_list, &f_run_summary, CUF_SuiteInactive,
				  0, _("Suite inactive"), _("BCUnit System"), pSuite, NULL);
	}
	...
	/* reach here if no suite initialization, or if it succeeded */
	  ...
	  if ((NULL != pSuite->pCleanupFunc) && (0 != (*pSuite->pCleanupFunc)())) {
		...
		add_failure(&f_failure_list, &f_run_summary, CUF_SuiteCleanupFailed,
					0, _("Suite cleanup failed."), _("BCUnit System"), pSuite, NULL);
...

{
  ...
  if (NULL != result) {
	snprintf(result, len, "%*s%*s%*s%*s%*s%*s%*s\n"   /* if you change this, be sure  */
			...
			(int)width[6], _("n/a"),
			(int)width[7], _("Elapsed time = "), (int)CU_get_elapsed_time(),  /* makes sure time is updated */
 ...
 *  run.  If the BCUnit framework is in an error condition after
		...
		if (CU_FALSE != pTest->fActive) {
		  ...
		  if (CU_FALSE != f_failure_on_inactive) {
			add_failure(&f_failure_list, &f_run_summary, CUF_TestInactive,
						0, _("Test inactive"), _("BCUnit System"), pSuite, pTest);
...
static CU_ErrorCode run_single_test(CU_pTest pTest, CU_pRunSummary pRunSummary)
{
  ...
  if (CU_FALSE != pTest->fActive) {
	...
	if (CU_FALSE != f_failure_on_inactive) {
	  add_failure(&f_failure_list, &f_run_summary, CUF_TestInactive,
				  0, _("Test inactive"), _("BCUnit System"), f_pCurSuite, f_pCurTest);
	}
	result = CUE_TEST_INACTIVE;
  }
...
static void test_CU_assertImplementation(void)
{
  ...
  pFailure5 = pFailure2;
  pFailure6 = pFailure3;
  TEST(0 != test_bcunit_get_n_memevents(pFailure4));
  TEST(test_bcunit_get_n_allocations(pFailure4) != test_bcunit_get_n_deallocations(pFailure4));
  TEST(0 != test_bcunit_get_n_memevents(pFailure5));
  TEST(test_bcunit_get_n_allocations(pFailure5) != test_bcunit_get_n_deallocations(pFailure5));
  TEST(0 != test_bcunit_get_n_memevents(pFailure6));
  TEST(test_bcunit_get_n_allocations(pFailure6) != test_bcunit_get_n_deallocations(pFailure6));
...
static void test_add_failure(void)
{
  ...
  if (TEST(NULL != pFailure1)) {
	...
	TEST(0 != test_bcunit_get_n_memevents(pFailure1));
	TEST(test_bcunit_get_n_allocations(pFailure1) != test_bcunit_get_n_deallocations(pFailure1));
  }
  ...
  if (TEST(NULL != pFailure1)) {
	...
	if (TEST(NULL != (pFailure2 = pFailure1->pNext))) {
	  ...
	  TEST(0 != test_bcunit_get_n_memevents(pFailure2));
	  TEST(test_bcunit_get_n_allocations(pFailure2) != test_bcunit_get_n_deallocations(pFailure2));
	}
  }
  ...
  TEST(0 == run_summary.nFailureRecords);
  TEST(0 != test_bcunit_get_n_memevents(pFailure3));
  TEST(test_bcunit_get_n_allocations(pFailure3) == test_bcunit_get_n_deallocations(pFailure3));
  TEST(0 != test_bcunit_get_n_memevents(pFailure4));
  TEST(test_bcunit_get_n_allocations(pFailure4) == test_bcunit_get_n_deallocations(pFailure4));
}
...
{
  test_bcunit_start_tests("TestRun.c");
  ...
  test_bcunit_end_tests();
}

#endif    /* BCUNIT_BUILD_TESTS */
