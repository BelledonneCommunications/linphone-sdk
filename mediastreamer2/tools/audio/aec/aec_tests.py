...
class AECTest:
    ...
    def __init__(self, test_name, **kwargs):
        ...
        if "test name in suite" not in kwargs:
            if test_name == "simple_talk_600ms":
                self.test_suite_name = "Simple talk 600ms delay"
            elif test_name == "simple_talk_white_noise":
    ...
    def move_files(self):
        ...
        audio_file_list = glob.glob("aec_output_ref.wav")
        if len(audio_file_list) > 0:
            ...
            os.rename(audio_file_name, self.files.aec_output_ref.file_name)
        else:
        ...
        if len(audio_file_list) > 0:
            ...
            os.rename(audio_file_name, self.files.aec_output_clean.file_name)
        else:
    ...
    def get_results(self):
        ...
        try:
            self.files.aec_output_clean.read_audio(self.files.sample_rate_hz)
            self.files.nearend.read_audio(self.files.sample_rate_hz)
            self.results = AudioAnalysis(self.files.nearend, self.files.aec_output_clean)
